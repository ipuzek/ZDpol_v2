...
library(knitr)
source("/IvanP/!Istrazivanja/Zadar_DZS/code.ZDpol/wrangling.R")
source("/IvanP/!Istrazivanja/Zadar_DZS/code.ZDpol/indicators.R")
source("/IvanP/R/medianordered.R")
```

#### tablica 1: stanovnici
...
kable(stan,caption = "Promjena broja stanovnika između popisa 2001. i 2011. godine")
...
kable(df_sumstan, caption = "Sume pomoćne 1")

df_brstan <- data.frame(
  data.rizaltZ %>%  filter(ZD.dijelovi == "poluotok") %>%  summarise(stan.ZDpol.2011 = mean(pop_2011)),
  data.rizaltZ %>%  filter(ZD.dijelovi == "poluotok") %>%  summarise(stan.ZDpol.2001 = mean(pop_2001)),
  data.rizaltZ %>%  filter(ZD.dijelovi == "poluotok" | ZD.dijelovi == "non_poluotok") %>%  summarise(stan.ZD.2011 = mean(pop_2011)),
  data.rizaltZ %>%  filter(ZD.dijelovi == "poluotok" | ZD.dijelovi == "non_poluotok") %>%  summarise(stan.ZD.2001 = mean(pop_2001))
  ) %>% round() %>% t() %>% as.data.frame()
kable(df_brstan, caption = "Prosjeci pomoćni 1.1")
...
data.frame(
  ...
  round(1) %>% as.data.frame() %>% kable(caption = "Prosjeci pomoćni 1.2")
```
...
kable(tmpa, caption = "Pokazatelji dobne strukture stanovništva zadarskog poluotoka")
...
kable(df_medijani, caption = "Prosjeci pomoćni 2.1")
...
kable(df_aging.index, caption = "Prosjeci pomoćni 2.2")
```
...
kable(tompas, caption = "Pokazatelji obrazovne strukture stanovništva zadarskog poluotoka")
...
kable(df_obr.index, caption = "pomoćni 3.1")
...
kable(df_obr.terc, caption = "pomoćni 3.2")
```
...
filter(data.rizaltZ, ZD.dijelovi == "poluotok") %>%
  ...
  round(1) %>%
  kable(caption = "Udjeli doseljenog stanovništva na zadarskom poluotoku")
...
df_migr <- data.frame(
  ...
  round(1) %>% as.data.frame()
kable(df_migr, caption = "pomoćni 4.1")
```

#### tablica 5: aktivnost i zaposlenost
...
filter(data.rizaltZ, ZD.dijelovi == "poluotok") %>%
  ...
  mutate(SK_MB = SK_MB / 100) %>%
  round(1) %>%
  kable(caption = "Pokazatelji zaposlenosti i aktivnosti stanovništva zadarskog poluotoka")
...
df_zap <- data.frame(
  ...
  round(1) %>% as.data.frame()
kable(df_zap, caption = "pomoćni 5.1")
...
df_akt <- data.frame(
  ...
  round(1) %>% as.data.frame()
kable(df_akt, caption = "pomoćni 5.2")
```
